    ...
    def elaborate(self, platform):
        ...
        with m.If(exec_enable & is_ctmm_op):
            with m.Switch(opcode):
                ...
                with m.Case(RV32CapOpcode.JALR):
                    m.d.comb += [
                        xr_wr_data.eq(nia_reg + 4),
                        xr_wr_en.eq(1),
                        jump_taken.eq(1),
                ...
                with m.Case(RV32CapOpcode.BRANCH):
                    ...
                    with m.Switch(funct3):
                        with m.Case(RV32CapFunct3Branch.BEQ):
                            m.d.comb += branch_taken.eq(rs1_val == rs2_val)
                        with m.Case(RV32CapFunct3Branch.BNE):
                            m.d.comb += branch_taken.eq(rs1_val != rs2_val)
                        with m.Case(RV32CapFunct3Branch.BLT):
                            m.d.comb += branch_taken.eq(rs1_s < rs2_s)
                        with m.Case(RV32CapFunct3Branch.BGE):
                            m.d.comb += branch_taken.eq(rs1_s >= rs2_s)
                        with m.Case(RV32CapFunct3Branch.BLTU):
                            m.d.comb += branch_taken.eq(rs1_val < rs2_val)
                        with m.Case(RV32CapFunct3Branch.BGEU):
                            m.d.comb += branch_taken.eq(rs1_val >= rs2_val)

                with m.Case(RV32CapOpcode.LOAD):
                    m.d.comb += [
                        dmem_addr_computed.eq(rs1_val + imm_i),
                        dmem_rd_en_sig.eq(1),
                    ]
                    with m.Switch(funct3):
                        with m.Case(RV32CapFunct3Load.LW):
                            m.d.comb += [xr_wr_data.eq(self.dmem_rd_data), xr_wr_en.eq(1)]
                        with m.Case(RV32CapFunct3Load.LH):
                        ...
                        with m.Case(RV32CapFunct3Load.LB):
                ...
                with m.Case(RV32CapOpcode.ARITHI):
                    ...
                    with m.Switch(funct3):
                        ...
                        with m.Case(RV32CapFunct3ArithI.SLTI):
                        ...
                        with m.Case(RV32CapFunct3ArithI.SLTIU):
                            m.d.comb += [xr_wr_data.eq(Mux(rs1_val < imm_ext, 1, 0)), xr_wr_en.eq(1)]
                        with m.Case(RV32CapFunct3ArithI.XORI):
                            m.d.comb += [xr_wr_data.eq(rs1_val ^ imm_ext), xr_wr_en.eq(1)]
                        with m.Case(RV32CapFunct3ArithI.ORI):
                            m.d.comb += [xr_wr_data.eq(rs1_val | imm_ext), xr_wr_en.eq(1)]
                        with m.Case(RV32CapFunct3ArithI.ANDI):
                            m.d.comb += [xr_wr_data.eq(rs1_val & imm_ext), xr_wr_en.eq(1)]
                        with m.Case(RV32CapFunct3ArithI.SLLI):
                            shamt = Signal(5)
                            m.d.comb += shamt.eq(imm_i[:5])
                            m.d.comb += [xr_wr_data.eq(rs1_val << shamt), xr_wr_en.eq(1)]
                        with m.Case(RV32CapFunct3ArithI.SRLI):
                            shamt_r = Signal(5)
                ...
                with m.Case(RV32CapOpcode.ARITH):
                    with m.Switch(funct3):
                        with m.Case(RV32CapFunct3Arith.ADD):
                            with m.If(funct7[5]):
                        ...
                        with m.Case(RV32CapFunct3Arith.SLL):
                            m.d.comb += [xr_wr_data.eq(rs1_val << rs2_val[:5]), xr_wr_en.eq(1)]
                        with m.Case(RV32CapFunct3Arith.SLT):
                        ...
                        with m.Case(RV32CapFunct3Arith.SRL):
                            ...
                            with m.Else():
                                m.d.comb += [xr_wr_data.eq(rs1_val >> rs2_val[:5]), xr_wr_en.eq(1)]
                        with m.Case(RV32CapFunct3Arith.OR):
                            m.d.comb += [xr_wr_data.eq(rs1_val | rs2_val), xr_wr_en.eq(1)]
                        with m.Case(RV32CapFunct3Arith.AND):
                            m.d.comb += [xr_wr_data.eq(rs1_val & rs2_val), xr_wr_en.eq(1)]

                with m.Case(RV32CapOpcode.FENCE):
                    pass

                with m.Case(RV32CapOpcode.SYSTEM):
                    pass
        ...
        # XR11 gt_type lives at bits [1:0] in the rv32_cap_amaranth layout
