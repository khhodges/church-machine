...
**How to apply:** Keep source-derived labels backward-compatible in the bridge/server/UI. Audit every TraceUnit opcode selector against the five-bit ISA field and keep the LUMP header offset in source maps. If arbitrary uploaded-code disassembly is required, add an explicitly versioned trace packet instruction-word field and reflash the board; do not infer it from event type.