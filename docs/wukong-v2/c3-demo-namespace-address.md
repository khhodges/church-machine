# C3 — Fix DEMO_NAMESPACE Containing Platform-Specific Absolute Addresses

## Priority
**Critical** — On the Wukong, NS slot 0 (Boot.NS) stores `word0_location = 0x1FC00`
(the previous platform's NS_TABLE_BASE). The Wukong's NS lives at **byte 0** of DMEM. Any program
...
## The Deeper Issue: `NS_TABLE_BASE` is Platform-Specific
`hw_types.py` defines `NS_TABLE_BASE = 0x1FC00` as a module-level constant, and
`boot_rom.py` uses it when building `DEMO_NAMESPACE`. This constant is platform-specific
(the previous platform places NS at the top of its DMEM; the Wukong places NS at byte 0).

The fix must not break the previous-platform path.
...
`boot_rom.py`. The default value remains `NS_TABLE_BASE` (for backward compatibility).
...
# Existing DEMO_NAMESPACE (previous-platform default — unchanged)
...
logic and makes the platform split explicit at the call site.

## Files to Change

| File | Change |
|------|--------|
| `hardware/boot_rom.py` | Refactor `_make_demo_namespace` into a function accepting `ns_table_base`; keep `DEMO_NAMESPACE` as the previous-platform default |
...
2. `build_demo_namespace(ns_table_base=NS_TABLE_BASE)` still produces the same bit-for-
   bit result as the current `DEMO_NAMESPACE` (previous-platform path unchanged).
...
- **Previous-platform regression**: the Previous-platform build must continue to use the original `DEMO_NAMESPACE`
  constant without any change to its byte values. Add a regression test that the
  previous-platform default `DEMO_NAMESPACE[0]` equals `0x1FC00 / 4` (word-addressed).
