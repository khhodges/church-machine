...
var _asmEditorNsIdx = null;   // NS slot currently inspected by CR detail
...
// Track the inspected NS context without transferring source ownership. The
// global editor belongs to its typed source owner (file, tab, example, LUMP,
// or buffer), never to whichever CR detail happens to be selected. Existing
// per-slot caches remain available to explicit patch operations.
...
function _clearPersistedStickyPatch(nsIdx) {
    ...
    // Clearing executable patch state does not clear a recoverable source
    // cache and never changes the independently-owned editor buffer.
}
...
function _showAsmErrors(errors, titleOverride, action) {
    var panel = document.getElementById('asmErrorPanel');
    if (!panel) return;
    ...
    if (action && action.label && typeof action.onClick === 'function') {
        html += '<div class="asm-error-panel-actions">'
              + '<button type="button" class="btn btn-sm asm-error-action-btn">'
              + _escHtml(action.label)
              + '</button></div>';
    }
    panel.innerHTML = html;
    var actionButton = panel.querySelector('.asm-error-action-btn');
    if (actionButton) {
...
function injectCRCode(logEl) {
    ...
    if (newCW > maxCW) {
        ...
        if (sim.bootComplete && sim.demoClistGTs && sim.demoClistGTs.length > 0) {
            ...
            const nsW1Updated = sim.memory[nsBase2 + 1] >>> 0;
    ...
    if (newCW !== oldCW) {
        ...
        const newW1 = sim.packNSWord1(newLimit17, w1f.gtSeq, w1f.g, w1f.f);
    ...
    if (petNameCaps && petNameCaps.length > 0) {
        ...
        for (let ci = 0; ci < petNameCaps.length; ci++) {
            ...
            if (clistAddr < sim.memory.length) {
                const w2 = sim.memory[sim._nsSlotBase(cap.nsSlot) + 2] >>> 0;
    ...
    for (const [nsIdxStr, patch] of entries) {
        ...
        if (newCW > maxCW2) {
            ...
            if (sim.bootComplete && sim.demoClistGTs && sim.demoClistGTs.length > 0) {
                ...
                const nsW1r    = sim.memory[nsBase2r + 1] >>> 0;
        ...
        const nsBase2     = sim._nsSlotBase(nsIdx2);
        ...
        // Preserve recoverable source in the slot cache, but never install it
        // into the independently-owned global editor.
        if (patch.src) {
