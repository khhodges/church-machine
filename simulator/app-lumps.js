...
function _displayLumpCapabilityName(cap, row) {
    var record = cap && typeof cap === 'object' ? cap : null;
    var rawName = record
        ? (record.dot_name || record.dotName || record.name || '')
        : String(cap || '');
    var upper = String(rawName).trim().toUpperCase();
    if (row === 0 && (upper === 'SELF' || upper === '__SELF__' ||
            (record && (record.symbolic_self === true ||
                record.compiler_owned_self === true)))) {
        return 'SELF';
    }
    return rawName || '\u2014';
}
    ...
    if (cc === 0) {
        ...
        for (let si = 0; si < cc; si++) {
            const cap  = _capsHaveSlots
                ...
                : (caps[si] || null);
            const name = _displayLumpCapabilityName(cap, si);
...
async function _fetchAndShowLumpTimeline(token, lump) {
    ...
    try {
        ...
        // Do not key archive rows by version alone. Frozen and legacy
        // artifacts can legitimately share a version number, and each file
        // must retain its own read-only Preview entry.
        const rows = history.map(h => ({
            ver: h.version,
            hist: h,
            tel: telByVer[h.version] || null,
        }));
        for (const tv of telRows) {
            if (!history.some(h => h.version === tv.lump_version)) {
                rows.push({ ver: tv.lump_version, hist: null, tel: tv });
            }
        }
        rows.sort((a, b) => b.ver - a.ver);
...
async function _setLumpHistoryCurrent(checkbox, token, version) {
    // A History checkbox is an activation request, not a second mutable
    // selection state. The protected restore flow writes a fresh current
    // revision and archives the previous live bytes atomically.
    if (!checkbox.checked) return;
    checkbox.checked = false;
    checkbox.disabled = true;
    try {
        await _restoreLumpFromHistory(token, version);
    } finally {
        // renderLumps() normally replaces this control after a successful
        // activation. Re-enable it when the transition was cancelled/failed.
        if (checkbox.isConnected) checkbox.disabled = false;
    }
}


function _lumpHistorySelectRow(rowEl, token, version, cw, cc, lumpSize, tk, historicalRecord, currentToken, archiveFilename, isCurrent) {
    ...
    _lumpHistoryPreview(token, version, cw, cc, lumpSize, tk, historicalRecord, currentToken, archiveFilename, isCurrent);
}
...
async function _lumpHistoryPreview(token, version, cw, cc, lumpSize, tk, historicalRecord, currentToken, archiveFilename, isCurrent) {
    ...
    try {
        const liveUrl = `/api/lump/${currentToken || token}/words`;
        ...
        t += '</tbody></table></div>';
        if (rawTailHex) {
            t += `<div class="lump-detail-section lump-history-raw-tail">` +
                `<div class="lump-section-title">Trailing raw bytes</div>` +
                `<code>${e(rawTailHex)}</code>` +
                `</div>`;
        }
        if (numWords === 0 && !rawTailHex) {
            t += `<div class="lump-detail-section">` +
                `<div class="lump-stored-src-empty">This LUMP file is empty.</div>` +
                `</div>`;
        }
        _showLumpHistoryPreviewModal(
...
async function openLumpInEditor(token, options) {
    ...
    var _serverFrameSource = _wordsResponse &&
        typeof _wordsResponse.source === 'string' && _wordsResponse.source.length > 0
    ...
    if (serverWords && serverWords.length > 0) {
        ...
        if (_lcfDec) {
            ...
            try {
                if (typeof _lcfDec.lumpInspectContentFrameSource === 'function') {
                    var _browserSourceInspection =
                        await _lcfDec.lumpInspectContentFrameSource(serverWords);
                    if (_browserSourceInspection &&
                            _browserSourceInspection.status === 'source') {
                        _browserFrameSource = _browserSourceInspection.source;
    ...
    if (!_inMemoryLump && _sourceResolution.restored &&
        ...
        var _openedSourceDigest = await crypto.subtle.digest(
            'SHA-256', new TextEncoder().encode(_sourceResolution.source));
...
// Loads the PostFlashSelftest LUMP (token 059dc47f) into the simulator, runs
