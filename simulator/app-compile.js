...
const LANG_EXAMPLE_GROUPS = {
    ...
    assembly:   ['ada_note_g', 'capability_test', 'system_patterns', 'compute_demo', 'led_control', 'led_dr_test', 'salvation', 'constants_dot', 'perm_attack', 'bind_attack', 'scheduler_pause', 'scheduler_yield', 'scheduler_wait', 'dijkstra_flag', 'post_flash_selftest', 'gt_v1_1_test', 'petname_demo', 'private_helpers', 'led_pattern', 'morse_cm_ok', 'old_boot'],
    ...
    haskell:    ['cloomc_church_math', 'cloomc_church_pair', 'cloomc_church_case', 'cloomc_sliderule_haskell', 'cloomc_dijkstra_flag_hs'],
...
const _CLOOMC_FILE_EXAMPLES = {
    ...
    'sliderule_haskell':       '/simulator/cloomc/sliderule_haskell.cloomc',
...
function _autoFillCapRights(caps) {
    ...
    for (const cap of caps) {
        ...
        if (/^Boot\.Thread$/i.test(cap.name) || /^Thread[.#]\d+$/i.test(cap.name)) continue;
...
abstraction DMABuffer {
    ...
    const sel = document.getElementById('langSelector');
    if (sel) {
        const isHaskell = ['church_math','church_pair','church_case','sliderule_haskell','dijkstra_flag_hs'].includes(name);
