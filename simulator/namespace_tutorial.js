class NamespaceTutorial {
    constructor() {
        ...
        this.NS_CW           = this.NS_SLOT_CAPACITY; // V2: plain 13-bit slot count (0..8191)
        this.NS_CC           = 0;                     // V2: reserved, must be zero
        this.NS_HEADER_WORDS = 16;
        this.NS_HEADER_START = 0;
    ...
    _headerRef() {
        const fields = [
            ...
            { bits: '[26:23]', name: 'n\u221213', val: String(Math.log2(this.TOTAL_WORDS) - 13), note: `Namespace-only RAM exponent: size = 2^(field+13) = ${this.TOTAL_WORDS} words`, w: 4,  bg: '#3a2000', border: '#c86000', text: '#f09040' },
            { bits: '[22:10]', name: 'cw',     val: String(this.NS_CW), note: `Plain Namespace slot count (0..8191): ${this.NS_SLOT_CAPACITY} fixed four-word entries`, w: 13, bg: '#002a40', border: '#2080c0', text: '#60b8f0' },
            ...
            { bits: '[7:0]',   name: 'cc',     val: String(this.NS_CC), note: 'Reserved in Namespace Header V2; must be zero (nonzero headers are rejected)', w: 8, bg: '#1a1000', border: '#b07820', text: '#f0c050' },
        ];
...
<p>Namespace Header V2 is a <strong>physical sixteen-word block</strong> at word ${hex(this.NS_HEADER_START)}, not synthesized display metadata. Word 0 uses the common 32-bit packing, but its Namespace meanings are selected by <code>typ=01</code>: <code>cw</code> is the plain slot count and <code>cc</code> is reserved and must be zero.</p>
...
<tr><td><code style="color:#f09040">n\u221213</code></td><td>[26:23]</td><td>4&nbsp;b</td><td><code>${Math.log2(this.TOTAL_WORDS) - 13}</code></td><td><code>namespaceSize = 2^(field+13)</code> = ${this.TOTAL_WORDS} words in this worked header</td></tr>
<tr><td><code style="color:#60b8f0">cw</code></td><td>[22:10]</td><td>13&nbsp;b</td><td><code>${this.NS_CW}</code></td><td><strong>Plain Namespace slot count</strong>: 0..8191; this header declares exactly <code>${this.NS_SLOT_CAPACITY}</code> fixed four-word entries, not code words</td></tr>
...
<tr><td><code style="color:#f0c050">cc</code></td><td>[7:0]</td><td>8&nbsp;b</td><td><code>0</code></td><td><strong>Reserved</strong>. V2 builders write zero and loaders reject nonzero values; it is not part of the slot count.</td></tr>
</table>
<div class="sr-key-concept"><div class="sr-concept-title">Address Space Layout \u2014 Four Zones</div>
<p>The namespace divides physical memory into four IDE-set zones. Lump zones grow <strong>upward \u2191</strong> from <code>0x0000</code>; the V20 NS Table occupies the top-of-memory byte range <code>0x1FC00\u20130x1FFFF</code>. Namespace capacity is <code>cw</code> plain slots, with ${this.NS_ENTRY_WORDS} words per entry; slot 0 is the highest-address entry and later slots descend by 16 bytes.</p>
...
<p>Physical V2 word order is: W0 header, W1 format marker <code>NSH2</code>, W2 base byte address (0), W3 table offset, W4 boot-entry byte address, W5 future seal boundary (6), then W6–W15 reserved zero words. The V20 NS Table begins at byte address <code>0x1FC00</code> (word address <code>0x7F00</code>) and ends at <code>0x1FFFF</code>. Each entry is 16 bytes: slot 0 begins at <code>0x1FFC0</code>, while slot 63 begins at <code>0x1FC00</code>.</p></div>
<div class="sr-key-concept"><div class="sr-concept-title">Encoding Formula</div>
<p><code>(0x1F &lt;&lt; 27) | (n_minus_13 &lt;&lt; 23) | (slot_count &lt;&lt; 10) | (0b01 &lt;&lt; 8) | 0</code></p>
<p>Example \u2014 65536-word namespace with 1024 slots (cw=1024, cc=0, n\u221213=3):</p>
<p><code style="color:#f0c050;font-size:1rem;">0xF818_0100</code>&nbsp;&nbsp;(magic=0x1F, n\u221213=3, cw=1024, typ=01, cc=0)</p></div>
...
<tr><td><code>cw</code></td><td>Code word count</td><td>Stack words (sw)</td><td>Plain slot count</td></tr>
<tr><td><code>cc</code></td><td>C-list GT count</td><td>Heap size (max N words)</td><td>Reserved; zero required</td></tr>
</table></div>`,
            },
            {
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = NamespaceTutorial;
}
