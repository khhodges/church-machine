...
const API_DATA = [
    ...
    },
    {
        ...
        methods: [
            ...
            { name: 'SecureObjectAdd',      signature: 'SecureObjectAdd(name, methods) → ownerPassKey', perms: 'E + M', description: 'Programmer-only: define a top-security object and issue its 128-bit-proof owner PassKey.', implemented: true },
            { name: 'SecureObjectMintPassKey', signature: 'SecureObjectMintPassKey(objectId, ownerKey, methods?) → PassKey', perms: 'E + owner key', description: 'Delegate a revocable, method-limited PassKey with an independent 128-bit proof.', implemented: true },
            { name: 'SecureObjectCall',     signature: 'SecureObjectCall(objectId, method, passKey) → result', perms: 'E + object key + proof', description: 'Call a protected top-security method only with an authorised object PassKey and its 128-bit proof.', implemented: true },
            { name: 'SecureObjectRevoke',   signature: 'SecureObjectRevoke(objectId, ownerKey, targetKey?) → ok', perms: 'E + owner key + proof', description: 'Revoke one delegated key or the entire top-security object.', implemented: true },
            { name: 'Bank.ObtainPassKey',   signature: 'Bank.ObtainPassKey(objectId, ownerKey) → PassKey', perms: 'E + owner key + proof', description: 'Obtain a fresh passkey for one of your stored objects without exposing its Namespace slot.', implemented: true },
    ...
    },
    {
        ...
        implemented: true, profile: 'Full',
        methods: [
            { name: 'Create',        signature: 'Create(lump: CR1) → CR0: Enter variable or NULL; DR0: status/error', perms: 'E + M + Inform R', description: 'Validate the complete self-defining LUMP before issuing its typed BankVariable Enter capability in CR0. CR0 is NULL on failure; DR0 is 1 on success or a specific Bank error code.', implemented: true },
            { name: 'Read',          signature: 'Read(variable: CR0, offset: DR1, words: DR2) → readable: CR4; DR0: status/error', perms: 'BankVariable E', description: 'Copy a bounded slice of a live Bank-managed LUMP through the typed CR0 capability into a fresh readable Inform capability; failures return a specific DR0 error code.', implemented: true },
            { name: 'InspectVariable', signature: 'InspectVariable(variable: CR0) → DR0: status/error; words: DR1; capacity: DR2; issue: DR3; lifecycle: DR4', perms: 'BankVariable E', description: 'Inspect scalar size/capacity/lifecycle fields in distinct DRs using only the typed CR0 capability; DR0 identifies failure and never carries authority.', implemented: true },
            { name: 'Release',       signature: 'Release(variable: CR0) → DR0: status/error', perms: 'BankVariable E', description: 'Zeroize and retire a Bank-managed LUMP through its CR0 capability; CR0 is cleared afterward and failures identify their error in DR0.', implemented: true },
            { name: 'RevokeVariable', signature: 'RevokeVariable(variable: CR0) → DR0: status/error', perms: 'BankVariable E', description: 'Revoke the CR0 variable authority and zeroize the private LUMP allocation; failures identify their error in DR0.', implemented: true },
            { name: 'MintKey',       signature: 'MintKey(capacity: DR1) → owner_key: CR1', perms: 'E + M', description: 'Create an empty dynamically allocated lockbox; return a typed BankOwnerKey capability in CR1 and status in DR0.', implemented: true },
            { name: 'Deposit',       signature: 'Deposit(owner_key: CR1, source: CR2, offset: DR1, words: DR2, kind: DR3) → status: DR0', perms: 'E + BankOwnerKey + Inform R', description: 'Copy a bounded LUMP or memory region into an empty lockbox using typed capabilities; reject raw GTs, proofs, and reconstructed handles supplied through DRs.', implemented: true },
            { name: 'Withdraw',      signature: 'Withdraw(owner_key: CR1) → valuable: CR2, status: DR0', perms: 'E + BankOwnerKey', description: 'Atomically release the valuable as a fresh typed Inform capability in CR2 and quarantine the lockbox key.', implemented: true },
            { name: 'Inspect',       signature: 'Inspect(owner_key: CR1) → metadata: DR0', perms: 'E + BankOwnerKey', description: 'Return safe custody state, size, type, and sequence metadata without revealing contents or Namespace location.', implemented: true },
            { name: 'Revoke',        signature: 'Revoke(owner_key: CR1) → status: DR0', perms: 'E + BankOwnerKey', description: 'Invalidate all lockbox keys and quarantine its Namespace-backed custody record.', implemented: true },
            { name: 'ObtainPassKey', signature: 'ObtainPassKey(owner_key: CR1) → owner_key: CR1, status: DR0', perms: 'E + BankOwnerKey + proof', description: 'Reissue a fresh object-scoped owner capability in CR1 after proving ownership with the current typed capability.', implemented: true },
            { name: 'ExportRecovery', signature: 'ExportRecovery(owner_key: CR1) → protectedState: DR0', perms: 'E + BankOwnerKey + proof', description: 'Create proof-bound authenticated recovery ciphertext in DR0 without persisting the raw PassKey proof.', implemented: true },
            { name: 'Recover',       signature: 'Recover(protectedState: DR1, original_key: CR1) → owner_key: CR1, status: DR0', perms: 'E + M + BankOwnerKey + proof', description: 'Restore a valuable into a fresh private Namespace entry and issue a replacement owner capability in CR1.', implemented: true },
            { name: 'List',          signature: 'List() → metadata: DR0', perms: 'E', description: 'List non-sensitive status metadata for active and revoked lockboxes in a scalar DR result.', implemented: true }
        ]
    },
    {
    ...
    {
        slot: null, token: 'b169bba4', name: 'Ethernet', layer: 2,
        perms: 'E',
        description: 'Raw Ethernet frame transport — XC7A100T lazy-load channel. Locator fetches all other abstractions through it. Identity: token b169bba4 (slot is assigned at load time and is not a stable identifier).',
