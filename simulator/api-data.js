...
const API_DATA = [
    ...
    },
    {
        ...
        implemented: true, profile: 'Full',
        methods: [
            { name: 'Create',        signature: 'Create(lump: CR1) → variable: CR3, status: DR0', perms: 'E + M + Inform R', description: 'Validate a complete self-defining LUMP from trusted bytes, then issue a typed BankVariable E capability; identity metadata is checked but never trusted as caller authority.', implemented: true },
            { name: 'Read',          signature: 'Read(variable: CR3, offset: DR1, words: DR2) → readable: CR4, status: DR0', perms: 'BankVariable E', description: 'Copy a bounded slice of a live Bank-managed LUMP into a fresh typed readable Inform capability.', implemented: true },
            { name: 'InspectVariable', signature: 'InspectVariable(variable: CR3) → status: DR0; words: DR1; capacity: DR2; issue: DR3; lifecycle: DR4', perms: 'BankVariable E', description: 'Inspect scalar size/capacity/lifecycle fields in distinct DRs, plus safe identity metadata, without revealing private Namespace storage.', implemented: true },
            { name: 'Release',       signature: 'Release(variable: CR3) → status: DR0', perms: 'BankVariable E', description: 'Zeroize and retire a Bank-managed LUMP after removing its private Namespace record.', implemented: true },
            { name: 'RevokeVariable', signature: 'RevokeVariable(variable: CR3) → status: DR0', perms: 'BankVariable E', description: 'Revoke the variable capability and zeroize the private LUMP allocation.', implemented: true },
