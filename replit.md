...
- **Hardware Target:** Primarily the Tang Nano 20K FPGA, with optional Efinix Ti60 F225 support, enabling features like CHANGE/SWITCH, SEAL_CHECK, FUSED_OPS, and GC. An IoT profile (`iot_profile=True`) removes GC, Lambda, Change, Switch, ELoadCall, XLoadLambda and replaces ChurchOutform with ChurchOutformIoT (lean tunnel-hunting, 8-byte header, ~9 FSM states). Build with `python -m hardware.gen_verilog --iot build` and `make -C hardware tang-iot`.
